...
Core routes remain under `/v1/`; Server 1.2 adds `/v1.2/` vocabulary, search and answer routes. Server 1.2.1 adds collection queries, governance and publication authorization under the same `/v1.2/` prefix. The REST plane listens on **:8080** in-container and is
...
- **Capability JWT**: a short-lived HS256 token minted by
  ...
  Data-plane credentials for sessions, search, checked answers and publication
  authorization (`query`), file ingestion (`ingest`), collection vocabulary
  management (`vocabulary`), and the `findings`/`evidence` service operations.
  Governance reads and writes require static `rw` credentials.
...
**uid contract:** every `/v1` or `/v1.2` request must carry `X-Munarium-Uid: <uid>` — the
...
Ledger query routes accept `?as_of_seq=N`.
...
Collection queries use `effective_on` to select business-effective publications;
this is separate from a ledger sequence pin. See the
[collection query contract](../guides/collection-vocabularies.md#collection-queries-and-publication-governance-121).
...
| | `GET /v1/sources/{source_id}` — metadata, never the bytes: `source_id`, `filename`, `media_type`, `content_hash`, `bytes_len`, `storage_backend`, `blob_uri`, `extraction_status`, `extraction_method`, `created_at` | `ServerApiService` |
...
All documented REST operations also have named native RPCs on "mmp.v1.ServerApiService" and methods in all four Server SDKs. See the [complete API guide](../../../clients/docs/guides/server-1.2.md) for the generated operation catalog and transport behavior.
...
| `GET /v1.2/collections/{id}/governance` | Static `rw` | Read retained publication metadata and query policy (1.2.1) |
| `PUT /v1.2/collections/{id}/governance` | Static `rw` | Append a revision-checked governance snapshot (1.2.1) |
| `POST /v1.2/query` | `query`, with collection clearance | Server selects governing publications, retrieves and composes (1.2.1) |
