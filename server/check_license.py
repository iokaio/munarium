...
"""The license gate for Munarium Server: Apache-2.0 wherever a tool reads a licence, the
...
EXEMPT = (
    ...
    "src/munarium-store-pg/migrations/",
    # the vendored Matrix contract: byte-identical to its lock; the Matrix tree owns it
    "contract/matrix/",
    # the MMP protos are published under Apache-2.0 by contract/mmp/publish.py, which
    # copies them verbatim; the canonical copies carry no header so the bundle's own
    # license rule applies to them
    "proto/",
    # the bundle's own LICENSE/NOTICE/README are the published artifact's, not this tree's
    "contract/mmp/LICENSE",
    "contract/mmp/NOTICE",
)
# Files that legitimately carry the Apache identifier (the published bundle's own texts
# and the publisher, which writes that line into the bundle it cuts):
FORBIDDEN_ALLOWED = ("contract/mmp/publish.py", "contract/mmp/README.md", "contract/mmp/NOTICE",
                     # third-party license texts quoted verbatim carry their own SPDX lines
